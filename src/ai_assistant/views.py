...
class ChatAPIView(APIView):
    ...
    def post(self, request):
        ...
        conversation_type = serializer.validated_data.get(
            "conversation_type", "policy_chat"
        )
    ...
    def _update_usage_metrics(self, user, metric_type):
        ...
        metrics, created = AIUsageMetrics.objects.get_or_create(
            user=user, date=today, defaults={metric_type: 1}
        )
        if not created:
...
class DocumentGenerationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if ai_response["success"]:
            ...
            document_title = (
                title or f"{document_type.replace('_', ' ').title()} - {policy.title}"
            )
            ...
            document = AIGeneratedDocument.objects.create(
                ...
                key_points=ai_response.get("document_structure", {}).get(
                    "key_points", []
                ),
