...
class AIConversationAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'user', 'conversation_type', 'related_policy_link',
        'message_count', 'model_used', 'is_active', 'created_at'
    ]
    list_filter = [
        'conversation_type', 'model_used', 'is_active', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('Conversation Data', {
            'fields': ('messages_display', 'context_data', 'message_count', 'last_message_time'),
            'classes': ('collapse',)
        }),
        ('Settings', {
            'fields': ('model_used', 'is_active')
        }),
        ('Timestamps', {
    ...
    def related_policy_link(self, obj):
        if obj.related_policy:
            url = reverse('admin:policy_tracking_policyrecommendation_change', 
                         args=[obj.related_policy.pk])
            return format_html('<a href="{}">{}</a>', url, obj.related_policy.title[:50])
        return '-'
    ...
    def messages_display(self, obj):
        if obj.messages:
            ...
            for msg in obj.messages[-5:]:  # Show last 5 messages
                ...
                formatted_messages.append(f"<strong>{role.title()}:</strong> {content}<br><small>{timestamp}</small>")
...
class AIInsightAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'insight_type', 'related_policy_link', 'confidence_level',
        'is_validated', 'view_count', 'usefulness_score', 'created_at'
    ]
    list_filter = [
        'insight_type', 'confidence_level', 'is_validated', 'model_used', 'created_at'
    ]
    ...
    fieldsets = (
        ...
        ('Content', {
            'fields': ('summary', 'content', 'key_points', 'recommendations', 'cultural_considerations')
        }),
        ('Quality & Validation', {
            'fields': ('confidence_level', 'model_used', 'is_validated', 'validated_by', 
                      'validation_notes', 'usefulness_score')
        }),
        ('Usage & Metadata', {
            'fields': ('view_count', 'tags', 'generated_by', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
    ...
    def mark_as_unvalidated(self, request, queryset):
        ...
        self.message_user(request, f'{queryset.count()} insights marked as unvalidated.')
...
class AIGeneratedDocumentAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'title', 'document_type', 'related_policy_link', 'status',
        'download_count', 'generated_by', 'created_at'
    ]
    list_filter = [
        'document_type', 'status', 'model_used', 'created_at'
    ]
    search_fields = ['title', 'content', 'related_policy__title']
    readonly_fields = ['download_count', 'generated_by', 'created_at', 'updated_at', 'content_preview']
    date_hierarchy = 'created_at'
    ...
    def content_preview(self, obj):
        if obj.content:
            ...
            return format_html('<div style="max-height: 200px; overflow-y: auto; white-space: pre-wrap;">{}</div>', preview)
        return 'No content'
...
class AIUsageMetricsAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'insights_generated', 'documents_created', 'total_tokens_used'
    ]
    ...
    fieldsets = (
        ...
        ('Usage Counts', {
            'fields': ('conversations_started', 'messages_sent', 'insights_generated', 'documents_created')
        }),
        ('Feature Usage', {
            'fields': ('policy_analysis_used', 'document_generation_used', 'cultural_guidance_used', 'evidence_review_used')
        }),
        ('Performance Metrics', {
