...
class DataValidator:
    ...
    @staticmethod
    def clean_decimal_value(value: Any, max_digits: int = 10, decimal_places: int = 2) -> Optional[Decimal]:
...
class FileProcessor:
    ...
    def get_file_headers(file_path: str) -> List[str]:
        ...
        if file_extension == 'csv':
            with open(file_path, 'r', encoding=FileProcessor.detect_file_encoding(file_path)) as file:
...
class MappingEngine:
    ...
    @staticmethod
    def suggest_mapping(source_fields: List[str], target_fields: List[str]) -> Dict[str, str]:
        ...
        for source_field in source_fields:
            ...
            for norm_target, target_field in normalized_target.items():
                if (normalized_source in norm_target or 
                    norm_target in normalized_source or
                    abs(len(normalized_source) - len(norm_target)) <= 2):
                    
                    # Simple similarity check
                    common_chars = set(normalized_source) & set(norm_target)
                    if len(common_chars) >= min(3, min(len(normalized_source), len(norm_target))):
        ...
        for field_name, value in record.items():
            ...
            if isinstance(field, models.EmailField):
                ...
                if value and len(str(value)) > field.max_length:
                    errors.append(f"Value too long for field '{field_name}' (max {field.max_length}): {value}")
