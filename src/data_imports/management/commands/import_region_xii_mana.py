...
from communities.models import OBCCommunity, CommunityLivelihood, CommunityInfrastructure, Stakeholder
from mana.models import (
    Assessment, AssessmentCategory, Need, NeedsCategory, GeographicDataLayer,
    BaselineStudy, BaselineStudyTeamMember
)
...
class Command(BaseCommand):
    ...
    def import_obc_communities(self, region, dry_run, update_existing):
        ...
        communities_data = [
            # SULTAN KUDARAT
            {
                ...
                'vulnerable_sectors': ['Senior Citizens', 'Children', 'Women', 'PWD', 'Solo Parents'],
                'challenges': ['Lack of access to healthcare', 'Education', 'Low income', 'Poor road accessibility']
            },
            {
                'province': 'Sultan Kudarat',
            ...
            },
            {
                ...
                'challenges': ['Lack of healthcare access', 'Low education', 'High poverty']
            },
            {
                'province': 'Sultan Kudarat',
                ...
                'challenges': ['Limited infrastructure', 'High poverty', 'Limited access to services']
            },
            {
                'province': 'Sultan Kudarat',
                ...
                'challenges': ['Lack of access to health', 'Education', 'Poverty', 'Discrimination']
            },
            ...
            # SARANGANI
            {
                ...
                'challenges': ['High poverty', 'Land conflicts', 'Rampant drug activities', 'Flooding']
            },
            {
                'province': 'Sarangani',
                'municipality': 'Glan',
                ...
                'challenges': ['Political discrimination', 'High poverty', 'Flooding', 'Drugs']
            },
            {
                'province': 'Sarangani',
                'municipality': 'Maasim',
                ...
                'challenges': ['Poverty', 'Natural hazards', 'Cultural erosion due to Westernization']
            },
            {
                'province': 'Sarangani',
                'municipality': 'Maitum',
                ...
                'vulnerable_sectors': ['Women', 'Children', 'Senior Citizens', 'PWD', 'Fisherfolk', 'Farmers'],
                'challenges': ['Poverty', 'Poor health services', 'Lack of potable water']
            },
            {
                'province': 'Sarangani',
                'municipality': 'Kiamba',
                ...
                'challenges': ['Presence of lawless elements', 'Peace and order issues']
            },
            ...
            # COTABATO PROVINCE (NORTH COTABATO)
            {
                ...
                'vulnerable_sectors': ['Farmers', 'Workers', 'Children', 'Elderly', 'Fisherfolk'],
                'challenges': ['Peace and order issues', 'Natural calamities', 'Unfinished infrastructure']
            },
            {
                'province': 'Cotabato',
                ...
                'challenges': ['Natural calamities', 'Social disorder', 'Limited access to services']
            },
            {
                'province': 'Cotabato',
                'municipality': 'Carmen',
                ...
                'population_estimate': 12000,
                'vulnerable_sectors': ['Farmers', 'Workers', 'Children', 'Elderly'],
                'challenges': ['Lack of opportunities', 'Land certification issues', 'Limited market access']
            },
            {
                'province': 'Cotabato',
                ...
                'population_estimate': 10000,
                'vulnerable_sectors': ['Farmers', 'Workers', 'Children', 'Elderly'],
                'challenges': ['Land ownership issues', 'Land certificate problems', 'Discrimination']
            },
            {
                'province': 'Cotabato',
            ...
            # SOUTH COTABATO
            {
                ...
                'vulnerable_sectors': ['Children', 'Women', 'PWD', 'Senior Citizens', 'Youth'],
                'challenges': ['Lack of equal opportunities', 'Limited access to services', 'Poverty']
            },
            {
                'province': 'South Cotabato',
                'municipality': 'Tupi',
            ...
            },
            {
                ...
                'population_estimate': 12000,
                'vulnerable_sectors': ['Children', 'Women', 'PWD', 'Senior Citizens', 'Youth'],
                'challenges': ['Lack of equal opportunities', 'Limited access to services']
            },
            ...
            # GENERAL SANTOS CITY
            {
                ...
                'challenges': ['Child labor', 'Limited access to education', 'Economic inequality']
            }
        ]
        ...
        for community_data in communities_data:
            try:
                ...
                municipality, _ = Municipality.objects.get_or_create(
                    ...
                    province=province,
                    defaults={
                        'code': community_data['municipality'].upper().replace(' ', '_'),
                        'municipality_type': 'city' if 'City' in community_data['municipality'] else 'municipality'
                    }
                )
                ...
                for barangay_name in community_data['barangays']:
                    if not dry_run:
                        ...
                        population_per_barangay = community_data['population_estimate'] // len(community_data['barangays'])
                        households_per_barangay = population_per_barangay // 5  # Assume 5 people per household
                        ...
                        if created:
                            imported_count += 1
                            self.stdout.write(f'Created OBC community: {barangay.name}, {municipality.name}')
                        elif update_existing:
                            for key, value in community_defaults.items():
                                setattr(community, key, value)
                            community.save()
                            updated_count += 1
                            self.stdout.write(f'Updated OBC community: {barangay.name}, {municipality.name}')
                    
                    else:
                        self.stdout.write(f'[DRY RUN] Would create OBC community: {barangay_name}, {community_data["municipality"]}')
                        imported_count += 1
            
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'Error importing {community_data["municipality"]}: {str(e)}')
                )
        
        self.stdout.write(
            self.style.SUCCESS(f'OBC Communities - Imported: {imported_count}, Updated: {updated_count}')
        )
    ...
    def create_mana_assessment(self, region, dry_run):
        ...
        if not sample_community:
            self.stdout.write(self.style.ERROR('No OBC communities found in Region XII. Please run the community import first.'))
            return
...
5. Secondary data analysis from government sources''',
                'study_domains': {
                    'demographic': ['Population size', 'Age structure', 'Household composition'],
                    'economic': ['Livelihood sources', 'Income levels', 'Employment status'],
                    'social': ['Education access', 'Health services', 'Social cohesion'],
                    'infrastructure': ['Housing', 'Water/sanitation', 'Transportation'],
                    'governance': ['Local leadership', 'Community participation', 'Service delivery']
                },
...
7. Strengthen cultural and religious identity preservation'''
        ...
        # Add team members
        team_members = [
            {'name': 'Norhan B. Hadji Abdullah', 'role': 'Overall Editor, Facilitator, and Writer', 'organization': 'OOBC'},
            {'name': 'Engr. Farhana U. Kabalu', 'role': 'Facilitator and Writer', 'organization': 'OOBC'},
            {'name': 'Esnain C. Mapait, RSW', 'role': 'Facilitator and Writer', 'organization': 'OOBC'},
            {'name': 'Ramla L. Manguda', 'role': 'Facilitator and Writer', 'organization': 'OOBC'},
            {'name': 'Michael A. Berwal, LPT', 'role': 'Facilitator and Writer', 'organization': 'OOBC'},
            {'name': 'Noron S. Andan', 'role': 'Executive Director', 'organization': 'OOBC'}
        ]
        ...
        for member_data in team_members:
            ...
            member, created = BaselineStudyTeamMember.objects.get_or_create(
                ...
                user=admin_user,
                role='principal_investigator' if 'Director' in member_data['role'] else 'co_investigator',
                defaults={
                    'responsibilities': f"{member_data['role']} - {member_data['organization']}"
                }
            )
            if created:
                self.stdout.write(f'Added team member: {member_data["name"]} as {member.role}')
        ...
        for need_data in priority_needs:
            ...
            need, created = Need.objects.get_or_create(
                ...
                defaults={
                    ...
                    'affected_households': need_data['affected_population'] // 5,  # Assume 5 people per household
