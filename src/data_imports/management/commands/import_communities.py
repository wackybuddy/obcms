...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--import-session', type=str, help='Import session ID for tracking')
        parser.add_argument('--mapping', type=str, help='JSON string with field mapping')
        parser.add_argument('--dry-run', action='store_true', help='Dry run without saving data')
        parser.add_argument('--update-existing', action='store_true', help='Update existing communities')
    ...
    def handle(self, *args, **options):
        ...
        if import_session_id:
            try:
                ...
                self.stdout.write(self.style.ERROR(f'Import session {import_session_id} not found'))
                return
            ...
            with open(csv_file, 'r', encoding='utf-8') as file:
                ...
                for row_number, row in enumerate(reader, start=2):  # Start at 2 to account for header
                    ...
                    try:
                        ...
                        community_name = row.get(field_mapping.get('name', ''), '').strip()
                        region_name = row.get(field_mapping.get('region', ''), '').strip()
                        province_name = row.get(field_mapping.get('province', ''), '').strip()
                        municipality_name = row.get(field_mapping.get('municipality', ''), '').strip()
                        barangay_name = row.get(field_mapping.get('barangay', ''), '').strip()
                        
                        if not all([community_name, region_name, province_name, municipality_name, barangay_name]):
                            log_message('warning', f'Missing required fields in row {row_number}', row_number, row)
                            skipped += 1
                            continue
                        ...
                        if not dry_run:
                            with transaction.atomic():
                                ...
                                region, _ = Region.objects.get_or_create(
                                    name=region_name,
                                    defaults={'code': region_name[:10], 'description': f'Auto-imported region: {region_name}'}
                                )
                                ...
                                community_data = {
                                    'population': self._safe_int(row.get(field_mapping.get('population', ''), '')),
                                    'households': self._safe_int(row.get(field_mapping.get('households', ''), '')),
                                    'cultural_background': row.get(field_mapping.get('cultural_background', ''), ''),
                                    'primary_language': row.get(field_mapping.get('primary_language', ''), ''),
                                    'established_year': self._safe_int(row.get(field_mapping.get('established_year', ''), '')),
                                    'settlement_type': row.get(field_mapping.get('settlement_type', ''), 'village'),
                                    'development_status': row.get(field_mapping.get('development_status', ''), 'developing'),
                                }
                                
                                # Remove None values
                                community_data = {k: v for k, v in community_data.items() if v is not None}
                                ...
                                if created:
                                    log_message('info', f'Created community: {community.name}', row_number)
                                    imported += 1
                                elif update_existing:
                                    for key, value in community_data.items():
                                        setattr(community, key, value)
                                    community.save()
                                    log_message('info', f'Updated community: {community.name}', row_number)
                                    updated += 1
                                else:
                                    log_message('warning', f'Community already exists: {community.name}', row_number)
                                    skipped += 1
                        else:
                            # Dry run - just validate
                            log_message('info', f'Would import community: {community_name}', row_number)
                            imported += 1
                        ...
                        log_message('error', f'Error importing row {row_number}: {str(e)}', row_number, row)
                        errors += 1
                        if import_session:
                            import_session.records_failed = errors
                            import_session.save()
