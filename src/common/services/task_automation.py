...
def create_tasks_from_template(template_name, **kwargs):
    ...
    from common.services.resource_bookings import create_bookings_for_task, ResourceBookingSpecError
    ...
    task_field_names = {
        field.name for field in StaffTask._meta.get_fields() if getattr(field, 'concrete', False)
    }
    task_kwargs = {key: value for key, value in kwargs.items() if key in task_field_names}
...
def create_milestone_tasks(sender, instance, created, **kwargs):
    ...
    StaffTask.objects.create(
        ...
        priority=StaffTask.PRIORITY_HIGH if getattr(instance, 'is_critical', False) else StaffTask.PRIORITY_MEDIUM,
