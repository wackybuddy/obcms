...
def rbac_dashboard(request):
    """
    Main RBAC dashboard with statistics and user management.

    Shows:
    - Total users, active roles, pending assignments, feature toggles
    - User list with search/filter capabilities
    - Quick access to role assignment and permissions
    """
    ...
    # Pending assignments (roles expiring soon or waiting approval)
    from datetime import timedelta
    soon_expiring = UserRole.objects.filter(
        is_active=True,
    ...
    # Get users for initial display
    users = User.objects.select_related('moa_organization').prefetch_related(
        'user_roles__role'
    ).filter(is_active=True).order_by('last_name', 'first_name')[:20]
    ...
    context = {
        ...
        'pending_assignments': soon_expiring,
    ...
    return render(request, 'common/rbac/dashboard.html', context)
...
def rbac_users_list(request):
    """
    HTMX endpoint for filtered users list.

    Provides real-time filtering and search for the users grid.
    """
    # Get filter parameters
    ...
    return render(request, 'common/rbac/partials/users_grid.html', context)
...
@login_required
@require_permission('oobc_management.manage_user_permissions')
def rbac_role_assignment_form(request, user_id):
    """
    Display role assignment form for a user (HTMX modal).
    """
    user = get_object_or_404(User, pk=user_id)
    ...
    return render(request, 'common/rbac/partials/role_assignment_form.html', context)


@login_required
@require_permission('oobc_management.assign_user_roles')
def rbac_bulk_assign_form(request):
    """
    Display bulk role assignment form (HTMX modal).
    """
    ...
    return render(request, 'common/rbac/partials/bulk_assign_modal.html', context)


@login_required
@require_permission('oobc_management.manage_user_permissions')
def rbac_permission_grant_form(request, user_id):
    """
    Display form to grant direct permission to user (HTMX).
    """
    user = get_object_or_404(User, pk=user_id)

    # Get available permissions
    organization = RBACService.get_user_organization_context(request)
    permissions = Permission.objects.filter(is_active=True).select_related('feature')

    if organization:
        permissions = permissions.filter(
            Q(feature__organization=organization) | Q(feature__organization__isnull=True)
        )
    ...
    return render(request, 'common/rbac/partials/permission_grant_form.html', context)
...
@require_POST
@require_permission('oobc_management.manage_user_permissions')
def rbac_permission_grant(request, user_id):
    """
    Grant a direct permission to a user (HTMX endpoint).
    """
    user = get_object_or_404(User, pk=user_id)
    ...
    if form.is_valid():
        ...
        try:
            ...
            return htmx_success_message(
                f"Successfully granted {permission.name} to {user.get_full_name()}",
...
@require_POST
@require_permission('oobc_management.manage_user_permissions')
def rbac_permission_remove(request, user_id, permission_id):
    """
    Remove a direct permission from a user (HTMX endpoint).
    """
    user = get_object_or_404(User, pk=user_id)
    ...
    try:
        with transaction.atomic():
            ...
            user_permission.save(update_fields=['is_active', 'updated_at'])
            ...
            log_model_change(
                ...
                user,
                'update',
                changes={'permission_removed': user_permission.permission.codename}
            )

        return htmx_success_message(
            f"Successfully removed permission from {user.get_full_name()}",
            target='#rbac-modal-content'
        )

    except Exception as e:
        return htmx_error_message(
            f"Failed to remove permission: {str(e)}",
            status=500
        )
...
@require_POST
@require_permission('oobc_management.assign_user_roles')
def rbac_bulk_remove_roles(request):
    """
    Remove all roles from selected users (HTMX endpoint).
    """
    user_ids = request.POST.get('user_ids', '').split(',')
    users = User.objects.filter(id__in=user_ids, is_active=True)

    if not users.exists():
        return htmx_error_message("No users selected.", status=400)
    ...
    try:
        with transaction.atomic():
            ...
            log_model_change(
                request,
                users.first(),  # Use first user as reference
                'update',
                changes={
                    'bulk_role_removal': {
                        'users_count': users.count(),
                        'roles_removed': removed_count
                    }
                }
            )

        return htmx_success_message(
            f"Successfully removed {removed_count} roles from {users.count()} user(s)",
            target='#users-grid'
        )

    except Exception as e:
        return htmx_error_message(
            f"Bulk removal failed: {str(e)}",
            status=500
        )
