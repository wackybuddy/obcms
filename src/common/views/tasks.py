...
def enhanced_task_dashboard(request):
    ...
    # Domain breakdown
    domain_stats = my_tasks.values('domain').annotate(
        count=Count('id')
    ).order_by('-count')
...
def task_analytics(request):
    ...
    status_counts_qs = StaffTask.objects.values('status').annotate(
        count=Count('id')
    )
...
def domain_task_analytics(request, domain):
    ...
    if phase_field:
        phase_breakdown = domain_tasks.values(phase_field).annotate(
            count=Count('id'),
            completed=Count('id', filter=Q(status=StaffTask.STATUS_COMPLETED)),
        ).order_by(phase_field)
    
    # Team workload (for this domain)
    team_workload = domain_tasks.values(
        'teams__name'
    ).annotate(
        count=Count('id')
    ).order_by('-count')
    ...
    # Assignee workload
    assignee_workload = domain_tasks.values(
        'assignees__username',
...
def task_template_list(request):
    ...
    templates = TaskTemplate.objects.filter(is_active=True).annotate(
