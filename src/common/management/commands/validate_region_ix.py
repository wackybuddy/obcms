...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--export',
            action='store_true',
            help='Export coordinate data to JSON file'
        )
    ...
    def _validate_region_coverage(self):
        ...
        if provinces_without_coords.exists():
            self.stdout.write(
                self.style.ERROR(f"  ❌ {provinces_without_coords.count()} provinces missing coordinates")
            )
        else:
            self.stdout.write(f"  ✅ All {provinces.count()} provinces have coordinates")

        # Check municipalities
        municipalities = Municipality.objects.filter(province__region__code='IX')
        municipalities_without_coords = municipalities.filter(center_coordinates__isnull=True)

        if municipalities_without_coords.exists():
            self.stdout.write(
                self.style.WARNING(f"  ⚠️  {municipalities_without_coords.count()} municipalities missing coordinates")
            )
        else:
            self.stdout.write(f"  ✅ All {municipalities.count()} municipalities have coordinates")
        ...
        if barangays_without_coords.exists():
            self.stdout.write(
                self.style.WARNING(f"  ⚠️  {barangays_without_coords.count()} barangays missing coordinates")
            )
        else:
            self.stdout.write(f"  ✅ All {barangays.count()} barangays have coordinates")
    ...
    def _validate_coordinate_accuracy(self):
        ...
        for level_name, model in [
            ...
            ('provinces', Province.objects.filter(region__code='IX')),
            ('municipalities', Municipality.objects.filter(province__region__code='IX')[:20]),  # Sample
            ('barangays', Barangay.objects.filter(municipality__province__region__code='IX')[:50])  # Sample
        ]:
            for obj in model:
                lat, lng = get_object_centroid(obj)
                if lat is not None and lng is not None:
                    # Check bounds
                    if not (expected_bounds['min_lat'] <= lat <= expected_bounds['max_lat'] and
                            expected_bounds['min_lng'] <= lng <= expected_bounds['max_lng']):
                        invalid_coords += 1
                        if self.verbose:
                            name = getattr(obj, 'name', str(obj))
                            self.stdout.write(
                                self.style.WARNING(f"    ⚠️  {name}: {lat:.6f}, {lng:.6f} outside expected bounds")
                            )
        ...
        if invalid_coords == 0:
            ...
            self.stdout.write(
                self.style.WARNING(f"  ⚠️  {invalid_coords} coordinates outside expected bounds")
            )
    ...
    def _validate_hierarchical_selection(self):
        ...
        test_cases = [
            ...
            ('municipality_level', Municipality.objects.filter(province__region__code='IX').first()),
            # Test barangay selection
            ('barangay_level', Barangay.objects.filter(municipality__province__region__code='IX').first())
        ]
        ...
        for test_name, test_obj in test_cases:
            if test_obj:
                ...
                if lat is not None and lng is not None:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"  ⚠️  {test_name}: No coordinates available")
                    )
    ...
    def _validate_form_integration_data(self):
        ...
        try:
            ...
            region_ix = next((r for r in location_data['regions'] if r['code'] == 'IX'), None)
            if region_ix:
                self.stdout.write("  ✅ Region IX found in location data")
                if self.verbose:
                    self.stdout.write(f"    Center: {region_ix.get('center_lat', 'N/A')}, {region_ix.get('center_lng', 'N/A')}")
            else:
                self.stdout.write(self.style.ERROR("  ❌ Region IX not found in location data"))

            # Check provinces
            region_ix_provinces = [p for p in location_data['provinces'] if p.get('region_id') == region_ix['id']]
            if len(region_ix_provinces) == 6:
                self.stdout.write(f"  ✅ All 6 provinces found in location data")
            else:
                self.stdout.write(
                    self.style.WARNING(f"  ⚠️  Only {len(region_ix_provinces)}/6 provinces in location data")
                )

            # Check sample municipalities
            if region_ix_provinces:
                sample_province_id = region_ix_provinces[0]['id']
                sample_municipalities = [m for m in location_data['municipalities']
                                       if m.get('province_id') == sample_province_id]
                self.stdout.write(f"  ✅ Sample province has {len(sample_municipalities)} municipalities in data")

            # Check sample barangays
            if 'barangays' in location_data:
                sample_barangays_count = len([b for b in location_data['barangays'][:100]
                                            if any(m['id'] == b.get('municipality_id')
    ...
    def _export_coordinate_data(self):
        ...
        for municipality in Municipality.objects.filter(province__region__code='IX')[:10]:
        ...
        for barangay in Barangay.objects.filter(municipality__province__region__code='IX')[:20]:
