...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if google_api_key:
            self.stdout.write(
                self.style.SUCCESS(
                    "✓ Google Maps API configured - using enhanced geocoding"
                )
            )
        else:
            self.stdout.write(
                self.style.WARNING(
                    "⚠ Google Maps API not configured - will use Nominatim only"
                )
            )
        ...
        if not barangays_only:
            ...
            for region in region_queryset:
                municipalities = Municipality.objects.filter(
                    province__region=region, is_active=True
                ...
                for municipality in municipalities:
                    ...
                    if dry_run:
                        self.stdout.write(
                            f"Would process: {municipality.name}, {municipality.province.name}"
                        )
                        total_processed += 1
                        continue
                    ...
                    if success:
                        ...
                        self.stdout.write(
                            f"✓ {municipality.name}, {municipality.province.name} ({source})"
                        )
                    else:
                        self.stdout.write(
                            self.style.WARNING(
        ...
        if not municipalities_only:
            ...
            for region in region_queryset:
                barangays = Barangay.objects.filter(
                    municipality__province__region=region, is_active=True
                ...
                for barangay in barangays:
                    ...
                    if dry_run:
                        self.stdout.write(
                            f"Would process: {barangay.name}, {barangay.municipality.name}"
                        )
                        total_processed += 1
                        continue
                    ...
                    if success:
                        ...
                        self.stdout.write(
                            f"✓ {barangay.name}, {barangay.municipality.name} ({source})"
                        )
                    else:
                        self.stdout.write(
                            self.style.WARNING(
        ...
        if dry_run:
            ...
            if google_count > 0:
                ...
                self.stdout.write(
                    f"Estimated Google API cost: ${estimated_cost:.2f} (after free tier)"
                )
    ...
    def process_location(self, location_obj, delay=0.2):
        ...
        try:
            ...
            lat, lng, updated, source = enhanced_ensure_location_coordinates(
                location_obj
            )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error processing {location_obj}: {str(e)}")
            )
            return False, "error"
