...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ('username', 'email', 'first_name', 'last_name', 'user_type', 
                   'organization', 'is_approved', 'is_active', 'date_joined')
    list_filter = ('user_type', 'is_approved', 'is_active', 'is_staff', 'is_superuser', 'date_joined')
...
class ProvinceAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'region', 'capital', 'municipality_count', 'is_active', 'created_at')
    ...
    fieldsets = (
        (None, {
            'fields': ('region', 'code', 'name', 'capital', 'is_active')
        }),
        ('Administrative Path', {
            'fields': ('full_path',),
            'classes': ('collapse',)
...
class MunicipalityAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'municipality_type', 'province', 'barangay_count', 'is_active', 'created_at')
    list_filter = ('municipality_type', 'province__region', 'province', 'is_active', 'created_at')
...
class BarangayAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'municipality', 'province', 'region', 'is_urban', 'is_active', 'created_at')
    list_filter = ('is_urban', 'is_active', 'municipality__province__region', 
                   'municipality__province', 'municipality', 'created_at')
    search_fields = ('code', 'name', 'municipality__name', 
                    'municipality__province__name', 'municipality__province__region__name')
    ordering = ('municipality__province__region__code', 'municipality__province__name', 
                'municipality__name', 'name')
    readonly_fields = ('created_at', 'updated_at', 'full_path', 'region', 'province')
    
    fieldsets = (
        (None, {
            'fields': ('municipality', 'code', 'name', 'is_urban', 'is_active')
        }),
        ('Administrative Path', {
            'fields': ('full_path', 'region', 'province'),
            'classes': ('collapse',)
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
