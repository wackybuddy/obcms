...
class ConversationalAssistant:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning("Gemini service not available - using rule-based responses only")
    ...
    def _generate_query_with_ai(
        ...
        # Build prompt
        prompt = self._build_query_generation_prompt(message, entities, available_models)
...
Example:
{"query": "BarangayOBC.objects.filter(province__name__icontains='Zamboanga').count()", "explanation": "Count communities in Zamboanga"}
        ...
        if 'how many' in message_lower or 'count' in message_lower:
            if 'communities' in entities or 'barangay' in message_lower:
                ...
                if 'zamboanga' in message_lower:
                    return "BarangayOBC.objects.filter(province__name__icontains='Zamboanga').count()"
                elif 'region' in message_lower:
                    return "BarangayOBC.objects.all().count()"
