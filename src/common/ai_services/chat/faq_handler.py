...
class FAQHandler:
    ...
    def __init__(self):
        ...
        self._base_pattern_cache = self._build_base_pattern_cache()
        self._sorted_enhanced_faqs = sorted(
            self.enhanced_faqs, key=lambda faq: faq.priority, reverse=True
        )
        self._enhanced_pattern_cache = self._build_enhanced_pattern_cache()
        self._base_pattern_lookup = {
            self._normalize_for_cache(pattern): (pattern, data)
            for pattern, data in self.base_faqs.items()
        }
    ...
    def try_faq(self, query: str) -> Optional[Dict]:
        ...
        if exact_match:
            response = self._build_response(
                exact_match, 1.0, start_time, matched_pattern=exact_match["pattern"]
            )
    ...
    def _normalize_for_cache(self, text: str) -> str:
        """Normalize a string for cache/preprocessing usage."""
    ...
    def _normalize_query(self, query: str) -> str:
        ...
        """
        return self._normalize_for_cache(query)

    def _build_base_pattern_cache(self) -> List[Dict]:
        """Prepare normalized base FAQ patterns for fast matching."""
        cache_entries = []
        for pattern, faq_data in self.base_faqs.items():
            normalized = self._normalize_for_cache(pattern)
    ...
    def _build_enhanced_pattern_cache(self) -> List[Dict]:
        """Prepare normalized enhanced FAQ patterns for fast matching."""
        cached_entries = []
        for faq in self._sorted_enhanced_faqs:
            pattern_entries = []
                ...
                pattern_entries.append(
                    {
                        ...
                        "matcher": SequenceMatcher(None, "", normalized),
                        "first_token": (
                            normalized.split(" ", 1)[0] if normalized else ""
                        ),
                        "length": len(normalized),
                        "tokens": set(normalized.split()),
                    }
                )
            cached_entries.append({"faq": faq, "patterns": pattern_entries})
        return cached_entries

    def _prepare_faq_payload(self, faq_data: Dict, pattern: str) -> Dict:
        """
        Create a normalized FAQ payload for downstream response building.

        Ensures expected keys are present even when sourced from legacy cache
        entries that may omit optional metadata.
        """
        payload = {
            "pattern": pattern,
        ...
        for optional_key in ("cache_key", "metadata", "faq_id", "priority"):
            if optional_key in faq_data:
                payload[optional_key] = faq_data[optional_key]
    ...
    def _exact_match(self, normalized_query: str) -> Optional[Dict]:
        ...
        """
        direct = self._base_pattern_lookup.get(normalized_query)
        if direct:
            pattern, faq_data = direct
            payload = self._prepare_faq_payload(faq_data, pattern)
            resolved_answer = self._get_faq_answer(faq_data)
            if resolved_answer is not None:
                payload["answer"] = resolved_answer
            if payload.get("answer") is not None or payload.get("cache_key"):
                return payload

        for entry in self._base_pattern_cache:
            pattern = entry["pattern"]
            normalized_pattern = entry["normalized"]
            faq_data = entry["faq_data"]

            if (
                normalized_query == normalized_pattern
                or normalized_query == pattern
                or normalized_query in normalized_pattern
                or normalized_pattern in normalized_query
            ):
    ...
    def _fuzzy_match(self, normalized_query: str) -> Optional[Dict]:
        ...
        query_first_token = (
            normalized_query.split(" ", 1)[0] if normalized_query else ""
        )
        ...
        for entry in self._base_pattern_cache:
            ...
            faq_data = entry["faq_data"]
            pattern = entry["pattern"]
            pattern_length = entry.get("length", len(normalized_pattern))
            pattern_first_token = entry.get("first_token", "")

            if abs(pattern_length - query_length) > 6:
                continue

            pattern_tokens = entry.get("tokens")
            if (
                pattern_first_token
                and query_first_token
                and pattern_first_token != query_first_token
                and pattern_tokens
                and query_tokens
                and not (pattern_tokens & query_tokens)
            ):
                continue
            ...
            if matcher.quick_ratio() < self.FUZZY_THRESHOLD * 0.9:
                continue
            ...
            if confidence >= self.FUZZY_THRESHOLD and confidence > best_confidence:
                ...
                if answer:
                    best_match = {
                        "pattern": pattern,
                        "faq": {
                            **self._prepare_faq_payload(faq_data, pattern),
                            "answer": answer,
                        },
    ...
    def _build_response(
        self, faq_data: Dict, confidence: float, start_time: float, matched_pattern: str
    ) -> Dict:
        """
    ...
    def add_faq(self, pattern: str, response: Dict):
        ...
        self.base_faqs[pattern_key] = response

        normalized = self._normalize_for_cache(pattern_key)
        self._base_pattern_cache.append(
            {
                "pattern": pattern_key,
                "normalized": normalized,
                "faq_data": response,
        ...
        self._base_pattern_lookup[normalized] = (pattern_key, response)
    ...
    def update_stats_cache(self) -> Dict[str, str]:
        ...
        try:
            ...
            try:
                ...
                # Top livelihoods
                top_livelihoods = (
                    OBCCommunity.objects.exclude(primary_livelihoods__isnull=True)
                    .exclude(primary_livelihoods="")
                    .values("primary_livelihoods")
                    .annotate(count=Count("id"))
                    .order_by("-count")[:5]
                )
                ...
                if top_livelihoods:
                    ...
                    stats["top_livelihoods"] = f"Top 5 livelihoods:\n{livelihood_list}"
            except ImportError:
            ...
            try:
                ...
                municipalities_count = Municipality.objects.filter(
                ...
                cities_count = Municipality.objects.exclude(
                    municipality_type="municipality"
                ).count()
                stats["cities_only"] = (
    ...
    def _try_enhanced_faq(self, normalized_query: str) -> Optional[Dict]:
        ...
        for entry in self._enhanced_pattern_cache:
            ...
            for pattern_entry in entry["patterns"]:
                normalized_pattern = pattern_entry["normalized"]
                pattern = pattern_entry["pattern"]
                pattern_first_token = pattern_entry.get("first_token", "")
                pattern_length = pattern_entry.get("length", len(normalized_pattern))
                pattern_tokens = pattern_entry.get("tokens", set())
    ...
    def get_faq_stats(self) -> Dict:
        ...
        enhanced_faqs_with_hits = len(
            [p for p in enhanced_hits if enhanced_hits[p] > 0]
        )
        ...
        return {
            ...
            "hit_rate": (
                round(total_faqs_with_hits / total_faqs * 100, 2)
                if total_faqs > 0
                else 0
            ),
            # Legacy FAQs
    ...
    def get_popular_enhanced_faqs(self, limit: int = 10) -> List[Dict]:
        ...
        # Sort by hit count
        sorted_faq_ids = sorted(hits.items(), key=lambda x: x[1], reverse=True)[:limit]
