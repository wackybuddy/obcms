...
class DocumentCommentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'document', 'document_title', 'user', 'user_name',
            'comment', 'parent_comment', 'is_internal', 'replies_count',
...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'category', 'category_name', 'file', 'file_url', 'download_url',
            'file_size', 'file_size_mb', 'file_type', 'file_extension',
            'community', 'community_name', 'tags', 'tag_list', 'author', 'language',
            'version', 'parent_document', 'is_latest_version',
            'confidentiality_level', 'confidentiality_level_display',
            'allowed_user_types', 'allowed_user_types_list', 'requires_approval',
            'status', 'status_display', 'uploaded_by', 'uploaded_by_name',
            'reviewed_by', 'reviewed_by_name', 'approved_by', 'approved_by_name',
            'document_date', 'expiry_date', 'is_expired', 'reviewed_at', 'approved_at',
            'download_count', 'view_count', 'is_featured', 'is_active',
            'is_image', 'is_pdf', 'is_office_document',
            'notes', 'comments', 'recent_access_logs', 'created_at', 'updated_at'
        ]
        extra_kwargs = {
    ...
    def get_download_url(self, obj):
        ...
        if request and obj.file and obj.can_be_accessed_by_user(request.user):
            ...
            return request.build_absolute_uri(
                reverse('documents_api:document-download', args=[obj.pk])
            )
        return None
    ...
    def get_comments(self, obj):
        ...
        if request and 'include_comments' in request.query_params:
            comments = obj.comments.filter(parent_comment__isnull=True).order_by('-created_at')[:5]
    ...
    def get_recent_access_logs(self, obj):
        ...
        request = self.context.get('request')
        if (request and 'include_access_logs' in request.query_params and 
...
class DocumentListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'category_name', 'community_name', 'file_size_mb', 'file_extension',
            'confidentiality_level', 'confidentiality_level_display',
            'status', 'status_display', 'uploaded_by_name', 'download_url',
            'view_count', 'download_count', 'is_featured', 'document_date',
            'created_at', 'updated_at'
        ]
...
class DocumentUploadSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Document
        fields = [
            'title', 'description', 'document_type', 'category', 'file',
            'community', 'tags', 'author', 'language', 'confidentiality_level',
            'allowed_user_types', 'requires_approval', 'document_date',
            'expiry_date', 'notes'
        ]
...
class DocumentVersionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Document
        fields = [
            'id', 'title', 'version', 'document_type', 'document_type_display',
            'file_size_mb', 'uploaded_by_name', 'is_latest_version',
            'created_at', 'updated_at'
        ]