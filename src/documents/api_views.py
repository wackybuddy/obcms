...
from .models import Document, DocumentCategory, DocumentAccess, DocumentComment
from .serializers import (
    DocumentSerializer, DocumentListSerializer, DocumentUploadSerializer,
    DocumentCategorySerializer, DocumentAccessSerializer, DocumentCommentSerializer
)
...
class DocumentViewSet(viewsets.ModelViewSet):
    ...
    def access_log(self, request, pk=None):
        ...
        access_logs = DocumentAccess.objects.filter(document=document).order_by('-accessed_at')
    ...
    def statistics(self, request):
        ...
        stats = {
            ...
            'by_confidentiality': queryset.values('confidentiality_level').annotate(count=Count('id')),
            'by_status': queryset.values('status').annotate(count=Count('id')),
            'total_downloads': sum(queryset.values_list('download_count', flat=True)),
            'total_file_size': sum(queryset.values_list('file_size', flat=True)),
        }
