...
def document_upload_path(instance, filename):
    ...
    now = timezone.now()
    community_part = f"community_{instance.community.id}" if instance.community else "general"
...
class Document(models.Model):
    ...
    is_featured = models.BooleanField(
        default=False,
        help_text="Whether this document is featured"
    )
    ...
    def allowed_user_types_list(self):
        ...
        if self.allowed_user_types:
            return [user_type.strip() for user_type in self.allowed_user_types.split(',') if user_type.strip()]
        return []
...
class DocumentAccess(models.Model):
    ...
    notes = models.TextField(
        blank=True,
        help_text="Additional notes about the access"
    )
...
class DocumentComment(models.Model):
    ...
    is_internal = models.BooleanField(
        default=False,
        help_text="Whether this is an internal comment"
    )
