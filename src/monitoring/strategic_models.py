...
class StrategicGoal(models.Model):
    ...
    target_outcome = models.TextField(
        blank=True, help_text="Specific, measurable outcome expected by target year"
    )
...
class AnnualPlanningCycle(models.Model):
    ...
    budget_approval_date = models.DateField(
        null=True, blank=True, help_text="Date when budget was approved"
    )
    ...
    plan_document_url = models.URLField(
        blank=True, help_text="Link to the annual plan document"
    )

    budget_document_url = models.URLField(
        blank=True, help_text="Link to the approved budget document"
    )
