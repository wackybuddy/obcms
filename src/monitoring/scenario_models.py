...
class BudgetScenario(models.Model):
    ...
    estimated_beneficiaries = models.PositiveIntegerField(
        default=0, help_text="Estimated number of beneficiaries under this scenario"
    )

    estimated_needs_addressed = models.PositiveIntegerField(
        default=0, help_text="Number of community needs addressed"
    )

    # Metadata
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_scenarios"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)

    notes = models.TextField(
        blank=True, help_text="Additional notes, assumptions, or context"
    )
    ...
    def optimization_weights_sum(self):
        ...
        return (
            self.weight_needs_coverage
            + self.weight_equity
            + self.weight_strategic_alignment
        )
    ...
    def recalculate_totals(self):
        ...
        self.allocated_budget = sum(alloc.allocated_amount for alloc in allocations)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_baseline and self.planning_cycle:
            ...
            BudgetScenario.objects.filter(
                planning_cycle=self.planning_cycle, is_baseline=True
...
class ScenarioAllocation(models.Model):
    ...
    priority_rank = models.PositiveIntegerField(
        null=True, blank=True, help_text="Priority ranking within scenario (1=highest)"
    )
    ...
    allocation_rationale = models.TextField(
        blank=True, help_text="Justification for this allocation amount"
    )
    ...
    def __str__(self):
        return (
            f"{self.scenario.name} → {self.ppa.title} (₱{self.allocated_amount:,.2f})"
        )
    ...
    def calculate_metrics(self):
        ...
        if self.ppa.target_beneficiaries and self.ppa.target_beneficiaries > 0:
            self.cost_per_beneficiary = (
                self.allocated_amount / self.ppa.target_beneficiaries
            )
        else:
        ...
        coverage_count = (
            self.ppa.municipality_coverage.count() + self.ppa.province_coverage.count()
        )
        self.equity_score = Decimal(
            str(coverage_count * 5)
        )  # 5 points per coverage unit

        # Strategic alignment score
        strategic_goals_count = self.ppa.contributing_strategic_goals.count()
        self.strategic_alignment_score = Decimal(
            str(strategic_goals_count * 15)
        )  # 15 points per goal
