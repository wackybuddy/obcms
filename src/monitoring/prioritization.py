...
def prioritization_matrix(request):
    ...
    for assessment in assessments:
        ...
        critical_needs = needs.filter(severity='critical').count() if hasattr(Need, 'severity') else 0
        high_needs = needs.filter(severity='high').count() if hasattr(Need, 'severity') else 0

        total_budget = linked_ppas.aggregate(
        ...
        matrix_data.append({
            ...
            'urgency': 'critical' if priority_score >= 10 else 'high' if priority_score >= 5 else 'medium',
        })
    ...
    for ppa in ppas:
        if ppa.sector:
            ...
            sector_coverage[ppa.sector]['total_budget'] += ppa.budget_allocation or Decimal('0.00')
    ...
    context = {
        ...
        'gap_percentage': (assessments_with_gaps / total_assessments * 100) if total_assessments > 0 else 0,
