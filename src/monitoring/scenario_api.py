...
def scenario_rebalance_budget(request):
    ...
    response_data = {
        ...
        'summary': {
            ...
            'average_allocation': float(total_allocated / len(results)) if results else 0,
            'utilization_rate': float((total_allocated / total_ceiling * 100)) if total_ceiling > 0 else 0,
        }
    }
...
def scenario_funding_mix(request):
    ...
    for source_key, source_label in MonitoringEntry.FUNDING_SOURCE_CHOICES:
        ...
        total = ppas.aggregate(total=models.Sum('budget_allocation'))['total'] or Decimal('0.00')
    ...
    total_current = sum(Decimal(str(d['current_total'])) for d in current_distribution.values())
    ...
    response_data = {
        ...
        'summary': {
            ...
            'diversification_index': len([c for c in changes if c['proposed_amount'] > 0]),
        }
    }
...
def scenario_obligation_forecast(request):
    ...
    target_obligation_rate = Decimal(str(request.data.get('target_obligation_rate', 85)))
    target_disbursement_rate = Decimal(str(request.data.get('target_disbursement_rate', 70)))

    # Get current rates
    ppas = MonitoringEntry.objects.all()
    total_allocation = ppas.aggregate(
    ...
    monthly_disbursement_needed = disbursement_gap / months_ahead if months_ahead > 0 else 0
