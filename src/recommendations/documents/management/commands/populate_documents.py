...
class Command(BaseCommand):
    ...
    def create_documents(self):
        ...
        for doc_data in documents:
            try:
                ...
                file_content = self.create_sample_file_content(
                    doc_data["title"], doc_data["file_type"]
                )
                ...
                document.updated_at = document.created_at + timedelta(
                ...
                self.stdout.write(
                    f'  Warning: Category "{doc_data["category"]}" not found'
                )
            except Exception as e:
                self.stdout.write(
                    f'  Error creating document "{doc_data["title"]}": {str(e)}'
                )
    ...
    def create_comments(self):
        ...
        documents = list(
            Document.objects.filter(confidentiality_level__in=["public", "internal"])
        )
        users = list(User.objects.all())
        ...
        for _ in range(30):
            ...
            comment = DocumentComment.objects.create(
                document=document, author=user, content=comment_text
            )
