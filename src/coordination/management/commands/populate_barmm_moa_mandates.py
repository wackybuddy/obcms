...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("Populating BARMM MOA mandates and functions...")
        )
        self.stdout.write("")
        ...
        for moa_name, data in BARMM_MOA_DATA.items():
            ...
            if not org:
                ...
                if "(" in moa_name:
                    try:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"  ⚠ Multiple organizations found with acronym from: {moa_name}"
                            )
                        )
                        continue
            ...
            if org:
                ...
                if not update_all and (org.mandate or org.powers_and_functions):
                    self.stdout.write(
                        self.style.WARNING(
                            f"  ⊘ Skipped: {org.name} (already has mandate/functions, use --update-all to override)"
                        )
                    )
                    skipped_count += 1
                    continue
