...
from .models import (
    Organization, OrganizationContact, Event, EventParticipant, ActionItem, EventDocument,
    Partnership, PartnershipSignatory, PartnershipMilestone, PartnershipDocument,
    Communication, CommunicationTemplate, CommunicationSchedule,
    StakeholderEngagementType, StakeholderEngagement, EngagementFacilitator,
    ConsultationFeedback, EngagementTracking
)
...
class OrganizationAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'organization_type', 'partnership_status', 'primary_contact_display', 
                   'address', 'partnership_count')
...
class OrganizationContactAdmin(admin.ModelAdmin):
    ...
    list_display = ('full_name', 'organization', 'position', 'email', 'phone', 
                   'is_primary', 'is_active')
    list_filter = ('is_primary', 'is_active', 'organization__organization_type')
    search_fields = ('first_name', 'last_name', 'email', 'organization__name', 'position')
...
class EventParticipantInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('name', 'organization', 'participation_role', 'response_status', 'satisfaction_rating')
...
class EventAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'event_type', 'start_date', 'venue', 'status', 
                   'participant_count', 'action_items_count')
    ...
    fieldsets = (
        ...
        ('Location', {
            'fields': ('is_virtual', 'venue', 'venue_details', 'virtual_meeting_link')
        }),
        ('Content', {
            'fields': ('description', 'objectives', 'agenda', 'target_participants')
        }),
        ('Logistics', {
            'fields': ('maximum_participants', 'registration_required', 'registration_deadline',
                      'budget', 'logistics_notes')
        }),
        ('Outcomes', {
            'fields': ('outcomes', 'follow_up_actions', 'lessons_learned')
        }),
        ('Metadata', {
    ...
    def action_items_count(self, obj):
        ...
        if pending > 0:
            return format_html('<span style="color: orange;">{} pending / {} total</span>', pending, total)
...
class EventParticipantAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'event', 'organization', 'participation_role', 'response_status', 'satisfaction_rating')
...
class ActionItemAdmin(admin.ModelAdmin):
    ...
    list_display = ('description_short', 'event', 'assigned_to', 'due_date', 'priority', 
                   'status', 'overdue_indicator')
...
class PartnershipAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'partnership_type', 'status', 'lead_organization', 
                   'start_date', 'milestone_progress')
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('title', 'partnership_type', 'status')
        }),
        ('Partners', {
            'fields': ('lead_organization', 'organizations', 'communities')
        }),
        ('Timeline', {
            'fields': ('start_date', 'end_date', 'duration_months')
        }),
        ('Content', {
            'fields': ('description', 'objectives', 'scope_of_work', 'expected_outcomes')
        }),
        ('Resources', {
            'fields': ('budget', 'funding_source', 'resource_commitments')
        }),
        ('Legal', {
            'fields': ('legal_framework', 'agreement_terms', 'renewal_conditions')
        }),
        ('Monitoring', {
            'fields': ('performance_indicators', 'reporting_schedule', 'evaluation_framework')
        }),
        ('Metadata', {
            'fields': ('notes', 'created_at', 'updated_at'),
            'classes': ('collapse',)
    ...
    inlines = [PartnershipSignatoryInline, PartnershipMilestoneInline, PartnershipDocumentInline]
    ...
    def milestone_progress(self, obj):
        ...
        return format_html(
            ...
            color, icon, completed, total, int(percentage)
        )
...
class CommunicationTemplateAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'template_type', 'subject_template', 'language', 'is_active')
...
class CommunicationAdmin(admin.ModelAdmin):
    ...
    list_display = ('subject', 'communication_type', 'sender', 'recipient_count', 
                   'communication_date', 'status')
...
class StakeholderEngagementAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'engagement_type', 'status', 'community_display', 
                   'planned_date', 'participant_count')
    ...
    fieldsets = (
        ...
        ('Target', {
            'fields': ('community', 'target_stakeholder_types', 'expected_participants')
        }),
        ('Schedule', {
            'fields': ('planned_date', 'duration_hours', 'venue')
        }),
        ('Content', {
            'fields': ('description', 'objectives', 'methodology', 'agenda')
        }),
        ('Preparation', {
            'fields': ('preparation_activities', 'materials_needed', 'logistics_requirements')
        }),
        ('Outcomes', {
            'fields': ('key_outcomes', 'recommendations', 'follow_up_actions')
        }),
        ('Evaluation', {
            'fields': ('satisfaction_rating', 'effectiveness_rating', 'lessons_learned')
        }),
        ('Metadata', {
            'fields': ('notes', 'created_at', 'updated_at'),
            'classes': ('collapse',)
...
class EngagementFacilitatorAdmin(admin.ModelAdmin):
    ...
    search_fields = ('user__username', 'user__first_name', 'user__last_name', 'engagement__title')
...
class EngagementTrackingAdmin(admin.ModelAdmin):
    ...
    list_display = ('community', 'period_type', 'period_start', 'period_end', 'calculated_at')
...
class PartnershipSignatoryAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'partnership', 'organization', 'position', 
                   'signature_date')
...
class PartnershipMilestoneAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'partnership', 'due_date', 'status', 'progress_percentage', 
                   'overdue_indicator')
...
class PartnershipDocumentAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'partnership', 'document_type', 'access_level_display', 'upload_date')
    ...
    def access_level_display(self, obj):
        ...
        if obj.is_confidential:
            return format_html('<span style="color: red; font-weight: bold;">🔒 CONFIDENTIAL</span>')
        else:
            return format_html('<span style="color: green; font-weight: bold;">📋 STANDARD</span>')
...
class CommunicationScheduleAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'schedule_type', 'recurrence_pattern', 'next_execution', 
                   'is_active', 'last_executed')
