...
def api_statistics(request):
    ...
    stats = {
        ...
        "communities": {
            ...
            "verified_stakeholders": Stakeholder.objects.filter(is_verified=True).count(),
        },
        "assessments": {
            "total_assessments": Assessment.objects.count(),
            "completed_assessments": Assessment.objects.filter(status='completed').count(),
            "ongoing_assessments": Assessment.objects.filter(status__in=['data_collection', 'analysis']).count(),
        ...
        "policy_tracking": {
            ...
            "implemented_policies": PolicyRecommendation.objects.filter(status='implemented').count(),
            "pending_review": PolicyRecommendation.objects.filter(status='under_review').count(),
        }
    }
