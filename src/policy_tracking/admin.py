...
from .models import (
    PolicyRecommendation, PolicyEvidence, PolicyImpact, PolicyDocument
)
...
class PolicyImpactInline(admin.TabularInline):
    ...
    model = PolicyImpact
    extra = 1
    fields = ('impact_type', 'title', 'measurement_type', 'baseline_value', 'target_value', 'current_value')
...
class PolicyRecommendationAdmin(admin.ModelAdmin):
    ...
    list_display = ('reference_number', 'title', 'category', 'status', 'priority', 
                   'proposed_by', 'submission_date', 'is_overdue_indicator')
    list_filter = ('status', 'priority', 'category', 'scope', 'submission_date', 
                   'approval_date', 'implementation_start_date')
    search_fields = ('title', 'reference_number', 'description', 'rationale',
                    'proposed_by__username', 'proposed_by__first_name', 'proposed_by__last_name')
    ordering = ('-created_at',)
    date_hierarchy = 'created_at'
    
    fieldsets = (
        ('Basic Information', {
            'fields': ('title', 'reference_number', 'category', 'priority', 'scope', 'status')
        }),
        ('Content', {
            'fields': ('description', 'rationale', 'problem_statement', 'policy_objectives', 
                      'proposed_solution', 'expected_outcomes')
        }),
        ('Timeline', {
            'fields': ('submission_date', 'review_deadline', 'approval_date', 
                      'implementation_start_date', 'implementation_deadline')
        }),
        ('Management', {
            'fields': ('proposed_by', 'lead_author', 'contributing_authors', 'assigned_reviewer')
        }),
        ('Implementation', {
            'fields': ('implementation_strategy', 'success_metrics', 'responsible_agencies',
                      'monitoring_framework', 'reporting_requirements')
        }),
        ('Impact Assessment', {
            'fields': ('potential_risks', 'mitigation_strategies', 'stakeholder_impact')
        }),
        ('Resources', {
            'fields': ('estimated_cost', 'funding_source', 'budget_implications',
                      'human_resources_required', 'technical_requirements')
        }),
        ('Legal & Regulatory', {
            'fields': ('legal_implications', 'regulatory_changes_needed', 'compliance_requirements')
        }),
        ('Review & Feedback', {
            'fields': ('review_comments', 'feedback_summary', 'revision_history')
        }),
        ('Outcomes & Evaluation', {
            'fields': ('actual_outcomes', 'lessons_learned', 'recommendations_for_improvement')
        }),
        ('Metadata', {
    ...
    filter_horizontal = ('target_communities', 'related_assessments', 'related_needs', 
...
class PolicyEvidenceAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'policy', 'evidence_type', 'reliability_level', 'verified', 'date_added')
    list_filter = ('evidence_type', 'reliability_level', 'verified', 'date_added', 'date_collected')
    ...
    fieldsets = (
        ...
        ('Documentation', {
            'fields': ('document', 'url', 'reference_citation')
        }),
        ('Verification', {
            'fields': ('verified', 'verified_by', 'verification_date', 'verification_notes')
        }),
        ('Timeline', {
            'fields': ('date_collected', 'date_added')
        }),
        ('Metadata', {
...
class PolicyImpactAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'policy', 'impact_type', 'measurement_type', 
                   'target_achievement_display', 'confidence_level')
    list_filter = ('impact_type', 'measurement_type', 'confidence_level', 'data_quality')
    ...
    fieldsets = (
        ...
        ('Measurement', {
            'fields': ('measurement_type', 'measurement_method', 'unit_of_measurement')
        }),
        ('Baseline & Targets', {
            'fields': ('baseline_value', 'baseline_date', 'target_value', 'target_date')
        }),
        ('Results', {
            'fields': ('current_value', 'last_measurement_date', 'final_value', 'final_assessment_date')
        }),
        ('Analysis', {
            'fields': ('analysis_notes', 'variance_explanation', 'contributing_factors')
        }),
        ('Quality', {
            'fields': ('data_quality', 'confidence_level')
        }),
        ('Metadata', {
            'fields': ('measured_by', 'notes'),
            'classes': ('collapse',)
        })
    )
...
class PolicyDocumentAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'policy', 'document_type', 'version', 'access_level_display', 
                   'author', 'upload_date')
    list_filter = ('document_type', 'is_confidential', 'is_public', 'upload_date', 'document_date')
    ...
    def access_level_display(self, obj):
        ...
        if obj.is_confidential:
            ...
            return format_html('<span style="color: orange; font-weight: bold;">👥 INTERNAL</span>')
