...
from .serializers import (
    PolicyRecommendationSerializer, PolicyRecommendationListSerializer,
    PolicyEvidenceSerializer, PolicyEvidenceListSerializer,
    PolicyImpactSerializer, PolicyImpactListSerializer,
...
class PolicyRecommendationViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = [
        'policy_category', 'status', 'priority_level', 'related_need', 
        'submitted_by', 'reviewed_by'
    ]
    search_fields = ['title', 'description', 'summary', 'reference_number']
    ordering_fields = [
        'title', 'submission_date', 'target_implementation_date', 
    ...
    def statistics(self, request):
        ...
        stats = {
            ...
            ).order_by('-submission_date')[:10].values(
                'id', 'title', 'status', 'submission_date'
            )
        }
...
class PolicyEvidenceViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'recommendation', 'evidence_type', 'is_verified', 'reliability_score'
    ]
