...
class PolicyRecommendation(models.Model):
    ...
    title = models.CharField(
        max_length=255,
        help_text="Title of the policy recommendation"
    )
    ...
    approval_date = models.DateField(
        null=True,
        blank=True,
        help_text="Date when policy was approved"
    )
    ...
    proposed_solution = models.TextField(
        help_text="Detailed proposed solution"
    )
    
    implementation_strategy = models.TextField(
        blank=True,
        help_text="Strategy for implementing the policy"
    )
    
    success_metrics = models.TextField(
        blank=True,
        help_text="Metrics for measuring policy success"
    )
    
    # Impact Assessment
    expected_outcomes = models.TextField(
        help_text="Expected outcomes and benefits"
    )
    
    potential_risks = models.TextField(
        blank=True,
        help_text="Potential risks and challenges"
    )
    
    mitigation_strategies = models.TextField(
        blank=True,
        help_text="Strategies to mitigate identified risks"
    )
    
    stakeholder_impact = models.TextField(
        blank=True,
        help_text="Impact on various stakeholders"
    )
    ...
    regulatory_changes_needed = models.TextField(
        blank=True,
        help_text="Required regulatory or legal changes"
    )
    
    compliance_requirements = models.TextField(
        blank=True,
        help_text="Compliance requirements and standards"
    )
    
    # Implementation
    implementation_phases = models.TextField(
        blank=True,
        help_text="Implementation phases and timeline"
    )
    
    responsible_agencies = models.TextField(
        blank=True,
        help_text="Agencies responsible for implementation"
    )
    
    monitoring_framework = models.TextField(
        blank=True,
        help_text="Framework for monitoring implementation"
    )
    ...
    review_comments = models.TextField(
        blank=True,
        help_text="Comments from reviewers"
    )
    ...
    def __str__(self):
        return f"{self.reference_number}: {self.title}" if self.reference_number else self.title
    
    def save(self, *args, **kwargs):
        if not self.reference_number:
            # Generate reference number
            year = timezone.now().year
            count = PolicyRecommendation.objects.filter(
...
class PolicyEvidence(models.Model):
    ...
    title = models.CharField(
        max_length=255,
        help_text="Title of the evidence"
    )
    ...
    description = models.TextField(
        help_text="Detailed description of the evidence"
    )
    
    source = models.CharField(
        max_length=255,
        blank=True,
        help_text="Source of the evidence"
    )
    ...
    key_findings = models.TextField(
        blank=True,
        help_text="Key findings from this evidence"
    )
    ...
    url = models.URLField(
        blank=True,
        help_text="URL to online source"
    )
    ...
    verification_notes = models.TextField(
        blank=True,
        help_text="Verification notes and comments"
    )
...
class PolicyImpact(models.Model):
    ...
    title = models.CharField(
        max_length=255,
        help_text="Title of the impact indicator"
    )
    
    description = models.TextField(
        help_text="Detailed description of the impact"
    )
    ...
    measurement_method = models.TextField(
        help_text="Method for measuring this impact"
    )
    ...
    baseline_date = models.DateField(
        null=True,
        blank=True,
        help_text="Date of baseline measurement"
    )
    ...
    last_measurement_date = models.DateField(
        null=True,
        blank=True,
        help_text="Date of last measurement"
    )
    ...
    analysis_notes = models.TextField(
        blank=True,
        help_text="Analysis of impact results"
    )
    ...
    contributing_factors = models.TextField(
        blank=True,
        help_text="Factors that contributed to the impact"
    )
