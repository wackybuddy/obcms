...
class CommunityLivelihoodInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('livelihood_type', 'specific_activity', 'households_involved', 
             'percentage_of_community', 'is_primary_livelihood', 'income_level')
...
class CommunityInfrastructureInline(admin.TabularInline):
    ...
    extra = 1
    fields = ('infrastructure_type', 'availability_status', 'coverage_percentage', 
             'condition', 'priority_for_improvement')
...
class StakeholderInline(admin.TabularInline):
    ...
    model = Stakeholder
    extra = 1
    fields = ('full_name', 'nickname', 'stakeholder_type', 'position', 
             'contact_number', 'influence_level', 'is_active')
...
class OBCCommunityAdmin(admin.ModelAdmin):
    ...
    list_display = ('barangay', 'municipality', 'province', 'region', 
                   'estimated_obc_population', 'households', 'development_status', 'is_active')
    list_filter = ('development_status', 'settlement_type', 'is_active',
                   'barangay__municipality__province__region',
                   'barangay__municipality__province',
                   'barangay__municipality',
                   'has_mosque', 'has_madrasah')
    search_fields = ('barangay__name', 'barangay__municipality__name',
                    'barangay__municipality__province__name',
                    'community_leader', 'community_names', 'obc_id')
    ...
    readonly_fields = ('created_at', 'updated_at', 'full_location', 
                      'total_age_demographics', 'average_household_size', 'percentage_obc_in_barangay', 'coordinates')
    
    inlines = [CommunityLivelihoodInline, CommunityInfrastructureInline, StakeholderInline]
    ...
    fieldsets = (
        ...
        ('Demographics', {
            'fields': (
                ('estimated_obc_population', 'total_barangay_population', 'percentage_obc_in_barangay'),
        ...
        ('Vulnerable Sectors', {
            'fields': (
                ...
                ('religious_leaders_ulama_count', 'csos_count', 'associations_count', 'teachers_asatidz_count')
            ),
            'classes': ('collapse',)
        }),
        ...
        ('Governance & Leadership', {
            'fields': (
                ('formal_political_representation', 'informal_leadership_structures'),
        ...
        ('Challenges & Barriers', {
            'fields': (
                ('governance_policy_challenges', 'access_public_services_challenges'),
...
class CommunityLivelihoodAdmin(admin.ModelAdmin):
    ...
    list_display = ('specific_activity', 'community', 'livelihood_type', 
                   'households_involved', 'percentage_of_community', 
                   'is_primary_livelihood', 'income_level', 'seasonal')
    list_filter = ('livelihood_type', 'is_primary_livelihood', 'seasonal', 
                   'income_level', 'community__development_status',
                   'community__barangay__municipality__province__region')
    search_fields = ('specific_activity', 'description',
...
class CommunityInfrastructureAdmin(admin.ModelAdmin):
    ...
    list_display = ('infrastructure_type', 'community', 'availability_status', 
                   'coverage_percentage', 'condition', 'priority_for_improvement')
    list_filter = ('infrastructure_type', 'availability_status', 'condition',
                   'priority_for_improvement', 'community__development_status',
                   'community__barangay__municipality__province__region')
    ...
    def mark_as_critical(self, request, queryset):
        ...
        self.message_user(request, f'{updated} infrastructure items marked as critical.')
    ...
    def mark_as_high_priority(self, request, queryset):
        ...
        self.message_user(request, f'{updated} infrastructure items marked as high priority.')
    ...
    def mark_as_low_priority(self, request, queryset):
        ...
        self.message_user(request, f'{updated} infrastructure items marked as low priority.')
...
class StakeholderAdmin(admin.ModelAdmin):
    ...
    list_display = ('display_name_with_type', 'community', 'stakeholder_type', 
                   'position', 'influence_level', 'engagement_level', 'contact_info_short',
                   'is_active', 'is_verified')
    list_filter = ('stakeholder_type', 'influence_level', 'engagement_level', 
                   'is_active', 'is_verified',
                   'community__barangay__municipality__province__region',
                   'community__development_status')
    search_fields = ('full_name', 'nickname', 'position', 'organization',
                    'community__barangay__name',
    ...
    fieldsets = (
        ...
        ('Influence & Engagement', {
            'fields': (
                ('influence_level', 'engagement_level'),
                'areas_of_influence'
            )
        }),
        ('Background', {
    ...
    actions = ['mark_as_verified', 'mark_as_unverified', 'mark_as_active', 'mark_as_inactive']
...
class StakeholderEngagementAdmin(admin.ModelAdmin):
    ...
    list_display = ('title', 'stakeholder', 'community', 'engagement_type', 
                   'date', 'outcome', 'follow_up_needed', 'participants_count')
    list_filter = ('engagement_type', 'outcome', 'follow_up_needed', 'date',
                   'stakeholder__stakeholder_type', 
                   'stakeholder__community__barangay__municipality__province__region')
    search_fields = ('title', 'description', 'stakeholder__full_name', 
                    'stakeholder__nickname', 'stakeholder__community__barangay__name',
                    'key_points', 'documented_by')
    ...
    def mark_follow_up_needed(self, request, queryset):
        ...
        self.message_user(request, f'{updated} engagements marked as needing follow-up.')
    ...
    def mark_follow_up_completed(self, request, queryset):
        ...
        self.message_user(request, f'{updated} engagements marked as follow-up completed.')
