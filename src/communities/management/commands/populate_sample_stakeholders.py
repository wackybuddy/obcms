...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS("Sample stakeholder data created successfully!")
        )
    ...
    def create_sample_stakeholders(self):
        ...
        if not communities.exists():
            self.stdout.write(
                self.style.ERROR(
                    "No communities found. Please run populate_sample_communities first."
                )
            )
            return
        ...
        for community_data in stakeholder_templates:
            try:
                ...
                for stakeholder_data in community_data["stakeholders"]:
                    ...
                    if (
                        "verification_date" not in stakeholder_data
                        and stakeholder_data.get("is_verified")
                    ):
                        stakeholder_data["verification_date"] = (
                    ...
                    if created:
                        self.stdout.write(
                            f"  Created: {stakeholder.display_name} ({stakeholder.get_stakeholder_type_display()})"
                        )
                    else:
                        self.stdout.write(
                            f"  Already exists: {stakeholder.display_name}"
                        )

            except OBCCommunity.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(
                        f'Community "{community_data["community_name"]}" not found. Skipping...'
                    )
                )
    ...
    def create_sample_engagements(self):
        ...
        if not stakeholders.exists():
            self.stdout.write(
                self.style.WARNING(
                    "No stakeholders found. Skipping engagement creation."
                )
            )
            return
        ...
        for stakeholder in stakeholders:
            ...
            for i in range(num_engagements):
                ...
                if engagement_data["follow_up_needed"]:
                    engagement_data["follow_up_date"] = engagement_date + timedelta(
                ...
                if created:
                    self.stdout.write(
                        f"  Created engagement: {engagement.title} for {stakeholder.display_name}"
                    )

        self.stdout.write(
            f"Created {StakeholderEngagement.objects.count()} sample engagements."
        )
