...
from communities.models import OBCCommunity, CommunityLivelihood, CommunityInfrastructure
    ...
    def create_sample_communities(self):
        ...
        for community_data in communities_data:
            try:
                ...
                community_data.pop('barangay_code')  # Remove barangay_code as it's not a model field
                ...
                if created:
                    ...
                    for livelihood_data in livelihoods_data:
                        ...
                        self.stdout.write(f'  - Added livelihood: {livelihood.specific_activity}')
                    ...
                    for infra_data in infrastructure_data:
                        ...
                        self.stdout.write(f'  - Added infrastructure: {infrastructure.get_infrastructure_type_display()}')
                else:
                    self.stdout.write(f'Community already exists: {community}')
                    
            except Barangay.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(f'Barangay {community_data["barangay_code"]} not found')
                )