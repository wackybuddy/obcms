...
def import_communities_csv(request):
    ...
    try:
        ...
        for index, row in df.iterrows():
            try:
                ...
                community_data = {
                    ...
                    'estimated_obc_population': row.get('estimated_obc_population') if pd.notna(row.get('estimated_obc_population')) else None,
                    'households': row.get('households') if pd.notna(row.get('households')) else None,
                    'primary_ethnolinguistic_group': row.get('primary_ethnolinguistic_group', ''),
        ...
        if errors:
            result['message'] = f'Processed {created_count + updated_count} communities with {len(errors)} errors'
        else:
            result['message'] = f'Successfully processed {created_count + updated_count} communities'
...
def _export_to_excel(communities, include_fields):
    ...
    field_mappings = {
        ...
        'demographics': [
            ('Children (0-12)', 'children_0_12'),
            ('Youth (13-30)', 'youth_13_30'),
    ...
    response['Content-Disposition'] = f'attachment; filename="obc_communities_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx"'
...
def _export_to_csv(communities, include_fields):
    ...
    response['Content-Disposition'] = f'attachment; filename="obc_communities_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
    
    writer = csv.writer(response)
    
    # Define basic fields for CSV (simplified)
    headers = [
        'OBC ID', 'Community Names', 'Region', 'Province', 'Municipality', 'Barangay',
        'Settlement Type', 'Population', 'Households', 'Primary Ethno Group',
        'Latitude', 'Longitude', 'Development Status'
    ]
...
def _export_to_pdf(communities, include_fields):
    ...
    response['Content-Disposition'] = f'attachment; filename="obc_communities_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf"'
    ...
    table_data = [['Community', 'Location', 'Population', 'Households', 'Development Status']]
    ...
    for community in communities[:50]:  # Limit to first 50 for PDF
        ...
        table_data.append([
            community.community_names[:30] + '...' if len(community.community_names or '') > 30 else community.community_names or 'N/A',
    ...
    data_table = Table(table_data, colWidths=[1.5*inch, 2*inch, 1*inch, 1*inch, 1.5*inch])
    ...
    if communities.count() > 50:
        note = Paragraph(f"<i>Note: Showing first 50 of {communities.count()} communities. Use Excel export for complete data.</i>", styles['Normal'])
        elements.append(Spacer(1, 20))
...
def _generate_summary_report(format_type):
    ...
    if format_type == 'pdf':
        response = HttpResponse(content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="obc_summary_report_{datetime.now().strftime("%Y%m%d")}.pdf"'
        ...
        regional_data = communities.values(
            'barangay__municipality__province__region__name'
        ).annotate(
            count=Count('id'),
            total_pop=Count('estimated_obc_population')
        ).order_by('-count')
        ...
        for item in regional_data:
            ...
            region_table_data.append([region_name, str(community_count), f'{region_pop:,}'])
        
        region_table = Table(region_table_data, colWidths=[3*inch, 1.5*inch, 1.5*inch])
        ...
        status_table = Table(status_table_data, colWidths=[2.5*inch, 1*inch, 1*inch])
        ...
        elements.append(Paragraph("<b>Development Status Distribution</b>", styles['Heading2']))
...
def _export_summary_to_excel(communities):
    ...
    response['Content-Disposition'] = f'attachment; filename="obc_summary_report_{datetime.now().strftime("%Y%m%d")}.xlsx"'
