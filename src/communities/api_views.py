...
class OBCCommunityViewSet(viewsets.ModelViewSet):
    ...
    """
    queryset = OBCCommunity.objects.filter(is_active=True).select_related(
        'barangay__municipality__province__region'
    ).prefetch_related('livelihoods', 'infrastructure')
    
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'development_status', 'settlement_type', 'has_mosque', 'has_madrasah',
        'barangay__municipality__province__region', 'barangay__municipality__province',
        'barangay__municipality', 'barangay', 'primary_language'
    ]
    search_fields = [
        'name', 'specific_location', 'primary_language', 'community_leader',
        'cultural_background', 'priority_needs', 'notes',
        'barangay__name', 'barangay__municipality__name', 
        'barangay__municipality__province__name'
    ]
    ordering_fields = [
        'name', 'population', 'households', 'established_year', 
        'development_status', 'created_at', 'updated_at'
    ]
    ordering = ['barangay__municipality__province__region__code', 
                'barangay__municipality__province__name',
                'barangay__municipality__name', 'barangay__name', 'name']
    ...
    def statistics(self, request):
        ...
        total_population = communities.aggregate(Sum('population'))['population__sum'] or 0
        total_households = communities.aggregate(Sum('households'))['households__sum'] or 0
        
        # Average household size
        avg_household_size = communities.exclude(
            households__isnull=True, population__isnull=True
        ).aggregate(
            avg_size=Avg('population') / Avg('households') if communities.exclude(
                households__isnull=True, population__isnull=True
            ).exists() else 0
        )['avg_size'] or 0
        
        # By region
        by_region = dict(communities.values(
            'barangay__municipality__province__region__name'
        ).annotate(
            count=Count('id'),
            population=Sum('population')
        ).values_list(
            'barangay__municipality__province__region__name', 'count'
        ))
        
        # By development status
        by_development_status = dict(communities.values('development_status').annotate(
            count=Count('id')
        ).values_list('development_status', 'count'))
        
        # By settlement type
        by_settlement_type = dict(communities.values('settlement_type').annotate(
            count=Count('id')
        ...
        stats_data = {
            ...
            'average_household_size': round(avg_household_size, 1) if avg_household_size else 0,
...
class CommunityLivelihoodViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'livelihood_type', 'is_primary_livelihood', 'seasonal', 'income_level',
        'community', 'community__development_status',
        'community__barangay__municipality__province__region'
    ]
    search_fields = ['specific_activity', 'description', 'challenges', 'opportunities']
    ordering_fields = ['livelihood_type', 'percentage_of_community', 'households_involved']
...
class CommunityInfrastructureViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'infrastructure_type', 'availability_status', 'condition', 'priority_for_improvement',
        'community', 'community__development_status',
        'community__barangay__municipality__province__region'
    ]
    search_fields = ['description', 'notes']
    ordering_fields = ['infrastructure_type', 'coverage_percentage', 'priority_for_improvement']
    ordering = ['community__name', 'infrastructure_type']
    
    @action(detail=False, methods=['get'])
    def critical_needs(self, request):
        """Get infrastructure with critical priority."""
        infrastructure = self.get_queryset().filter(
...
class StakeholderViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'stakeholder_type', 'influence_level', 'engagement_level', 
        'is_active', 'is_verified', 'community',
        'community__development_status', 'community__barangay__municipality__province__region'
    ]
    search_fields = [
        'full_name', 'nickname', 'position', 'organization',
        'contact_number', 'email', 'areas_of_influence',
        'special_skills', 'community__name'
    ]
    ordering_fields = [
        'full_name', 'stakeholder_type', 'influence_level', 
        'engagement_level', 'since_year', 'created_at'
    ]
    ...
    def statistics(self, request):
        ...
        # By type
        by_type = dict(stakeholders.values('stakeholder_type').annotate(
            count=Count('id')
        ).values_list('stakeholder_type', 'count'))
        
        # By influence level
        by_influence_level = dict(stakeholders.values('influence_level').annotate(
            count=Count('id')
        ).values_list('influence_level', 'count'))
        
        # By engagement level
        by_engagement_level = dict(stakeholders.values('engagement_level').annotate(
            count=Count('id')
        ).values_list('engagement_level', 'count'))
        
        # By community
        by_community = dict(stakeholders.values('community__name').annotate(
            count=Count('id')
    ...
    def community_leaders(self, request):
        ...
        stakeholders = self.get_queryset().filter(
            stakeholder_type__in=['community_leader', 'barangay_captain', 'tribal_leader']
        )
        serializer = self.get_serializer(stakeholders, many=True)
        return Response(serializer.data)
...
class StakeholderEngagementViewSet(viewsets.ModelViewSet):
    ...
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'engagement_type', 'outcome', 'follow_up_needed', 'stakeholder',
        'stakeholder__stakeholder_type', 'stakeholder__community',
        'stakeholder__community__development_status'
    ]
    search_fields = [
        'title', 'description', 'key_points', 'action_items',
        'stakeholder__full_name', 'stakeholder__nickname',
        'documented_by', 'location'
    ]
