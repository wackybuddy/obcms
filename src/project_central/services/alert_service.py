...
class AlertService:
    ...
    def generate_daily_alerts(cls):
        ...
        try:
            ...
            logger.error(f"Error generating approval bottleneck alerts: {e}", exc_info=True)
            results['errors'].append(f"approval_bottlenecks: {str(e)}")

        try:
            results['disbursement_delays'] = cls.generate_disbursement_delay_alerts()
        except Exception as e:
            logger.error(f"Error generating disbursement delay alerts: {e}", exc_info=True)
        ...
        try:
            ...
            logger.error(f"Error generating workflow blocked alerts: {e}", exc_info=True)
            results['errors'].append(f"workflow_blocked: {str(e)}")

        results['total'] = sum(v for k, v in results.items() if isinstance(v, int))

        logger.info(f"Daily alert generation complete. Total alerts: {results['total']}")
        return results
    ...
    def get_alert_summary(cls):
        ...
        summary = {
            ...
            'by_severity': dict(
                active_alerts.values('severity').annotate(
                    count=Count('id')
                ).values_list('severity', 'count')
            ),
            'by_type': dict(
                active_alerts.values('alert_type').annotate(
                    count=Count('id')
