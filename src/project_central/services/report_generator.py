...
class ReportGenerator:
    ...
    def generate_portfolio_report(cls, fiscal_year=None, output_format='dict'):
        ...
        cost_effectiveness = AnalyticsService.get_cost_effectiveness_metrics(fiscal_year=fiscal_year)
        ...
        report_data = {
            ...
            'summary': {
                ...
                'budget_utilization': utilization['ppa_utilization']['disbursement_rate'],
    ...
    @classmethod
    def generate_budget_utilization_report(cls, fiscal_year=None, sector=None, output_format='dict'):
        """
        ...
        for ppa in queryset:
            ...
            if ppa.budget_allocation and ppa.budget_allocation > 0:
                obligation_rate = (ppa.total_obligated or 0) / ppa.budget_allocation * 100
                disbursement_rate = (ppa.total_disbursed or 0) / ppa.budget_allocation * 100
    ...
    def generate_workflow_progress_report(cls, fiscal_year=None, output_format='dict'):
        ...
        for workflow in queryset:
            days_in_current_stage = (timezone.now().date() - workflow.last_stage_change).days if workflow.last_stage_change else 0
            ...
            workflow_details.append({
                ...
                'project_lead': workflow.project_lead.get_full_name() if workflow.project_lead else 'Unassigned',
                'mao_focal_person': workflow.mao_focal_person.name if workflow.mao_focal_person else 'Not assigned',
    ...
    def generate_cost_effectiveness_report(cls, fiscal_year=None, output_format='dict'):
        ...
        metrics = AnalyticsService.get_cost_effectiveness_metrics(fiscal_year=fiscal_year)
    ...
    def _export_budget_utilization_to_csv(cls, report_data):
        ...
        writer.writerow([
            'Title', 'Sector', 'Funding Source', 'Budget Allocation',
            'Total Obligated', 'Total Disbursed', 'Obligation Rate %',
            'Disbursement Rate %', 'Status', 'Progress %'
        ])
        ...
        writer.writerow([
            'Name', 'Ceiling Amount', 'Allocated Amount',
            'Utilization %', 'Remaining Amount', 'Enforcement Level'
        ])
    ...
    def _export_workflow_progress_to_csv(cls, report_data):
        ...
        writer.writerow([
            'Need Title', 'Current Stage', 'Priority', 'Project Lead',
            'MAO Focal Person', 'On Track', 'Blocked', 'Days in Stage',
            'Target Completion', 'Overdue', 'PPA', 'Estimated Budget'
        ])
    ...
    def _export_cost_effectiveness_to_csv(cls, report_data):
        ...
        writer.writerow([
            'Sector', 'Total Budget', 'Total Beneficiaries',
            'Avg Cost per Beneficiary', 'Project Count'
        ])
