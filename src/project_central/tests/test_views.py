...
pytest.skip(
    "Project Central view tests require legacy StaffTask/TaskTemplate models removed in WorkItem refactor.",
...
from project_central.forms import ProjectWorkflowFromPPAForm
    ...
    def test_generate_workflow_tasks_creates_records(self):
        ...
        if response.status_code != 302:
            if hasattr(response, "context") and response.context is not None:
                ...
                print("NO CONTEXT AVAILABLE", response)
...
class MoaPPAIntegrationTests(TestCase):
    """Ensure MOA PPA management is accessible from Project Central."""

    def setUp(self):
        self.user = User.objects.create_user(
            username="ppa_manager",
    ...
    def _create_need(self):
        ...
        return Need.objects.create(
            ...
            description="Rehabilitation of community irrigation system.",
            category=category,
            community=community,
    ...
    def _create_moa_ppa(self, **overrides):
        defaults = {
    ...
    def test_moa_ppa_list_view_shows_entries(self):
        ppa = self._create_moa_ppa()

        response = self.client.get(reverse("project_central:moa_ppa_list"))

        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "project_central/ppas/list.html")
        self.assertContains(response, ppa.title)

    def test_create_moa_ppa_creates_entry(self):
        payload = {
            "implementing_moa": str(self.moa.id),
            "title": "Livelihood Starter Kits",
            "summary": "Starter kits for farmer cooperatives",
            "sector": MonitoringEntry.SECTOR_ECONOMIC,
            "appropriation_class": MonitoringEntry.APPROPRIATION_CLASS_MOOE,
            "funding_source": MonitoringEntry.FUNDING_SOURCE_GAA,
            "status": "planning",
        ...
        response = self.client.post(reverse("project_central:moa_ppa_create"), payload)

        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            MonitoringEntry.objects.filter(title="Livelihood Starter Kits").exists()
        )

    def test_edit_moa_ppa_updates_record(self):
        ppa = self._create_moa_ppa(summary="Initial summary")
        ...
        payload = {
            ...
            "sector": ppa.sector,
            "appropriation_class": ppa.appropriation_class,
        ...
        response = self.client.post(
            reverse("project_central:moa_ppa_edit", args=[ppa.id]), payload
        )

        self.assertEqual(response.status_code, 302)
        ppa.refresh_from_db()
        self.assertEqual(ppa.summary, "Revised summary")
        self.assertEqual(ppa.status, "ongoing")

    def test_delete_moa_ppa_removes_record(self):
        ppa = self._create_moa_ppa()

        response = self.client.post(
            reverse("project_central:moa_ppa_delete", args=[ppa.id])
        )

        self.assertEqual(response.status_code, 302)
        self.assertFalse(MonitoringEntry.objects.filter(id=ppa.id).exists())

    def test_create_workflow_from_ppa_links_records(self):
        ppa = self._create_moa_ppa()

        payload = {
            "primary_need": str(self.need.id),
            "priority_level": ProjectWorkflow.PRIORITY_LEVELS[1][0],
            "project_lead": str(self.user.id),
            "estimated_budget": "750000",
            "notes": "Kick-off workflow",
        }

        form = ProjectWorkflowFromPPAForm(ppa, payload)
        self.assertTrue(form.is_valid(), form.errors)

        response = self.client.post(
            reverse("project_central:create_workflow_from_ppa", args=[ppa.id]), payload
        )

        self.assertEqual(response.status_code, 302)
        ppa.refresh_from_db()
        self.assertIsNotNone(ppa.project_workflow)
        self.assertEqual(ppa.project_workflow.primary_need, self.need)

    def test_create_project_workflow_redirects_to_moa_list(self):
        response = self.client.get(reverse("project_central:create_project_workflow"))

        self.assertEqual(response.status_code, 302)
        self.assertTrue(
            response.url.endswith(reverse("project_central:moa_ppa_list"))
        )
