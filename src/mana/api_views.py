...
from .models import (
    ...
    BaselineStudy, BaselineStudyTeamMember, BaselineDataCollection,
    BaselineIndicator
)
from .serializers import (
    AssessmentCategorySerializer, AssessmentSerializer, AssessmentListSerializer,
    AssessmentTeamMemberSerializer, SurveySerializer, SurveyQuestionSerializer,
    SurveyResponseSerializer, MappingActivitySerializer, NeedsCategorySerializer,
    NeedSerializer, NeedListSerializer, NeedsPrioritizationSerializer,
    NeedsPrioritizationItemSerializer, GeographicDataLayerSerializer,
    MapVisualizationSerializer, SpatialDataPointSerializer,
    BaselineStudySerializer, BaselineStudyListSerializer,
    BaselineStudyTeamMemberSerializer, BaselineDataCollectionSerializer,
...
class AssessmentViewSet(viewsets.ModelViewSet):
    ...
    queryset = Assessment.objects.all().select_related('category', 'community', 'lead_assessor')
    permission_classes = [permissions.IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
...
class NeedViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['category', 'priority_level', 'urgency_level', 'status', 'assessment']
...
class BaselineStudyViewSet(viewsets.ModelViewSet):
    ...
    queryset = BaselineStudy.objects.all().select_related('community', 'lead_researcher')
    permission_classes = [permissions.IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
...
class NeedsPrioritizationViewSet(viewsets.ModelViewSet):
    ...
    queryset = NeedsPrioritization.objects.all().select_related('assessment', 'facilitator')
...
class BaselineDataCollectionViewSet(viewsets.ModelViewSet):
    queryset = BaselineDataCollection.objects.all().select_related('indicator', 'collected_by')
