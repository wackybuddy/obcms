...
from .models import (
    Assessment, AssessmentCategory, AssessmentTeamMember,
    WorkshopActivity, WorkshopSession, WorkshopParticipant, WorkshopOutput,
    MANAReport, Survey, Need, NeedsCategory
)
...
def new_assessment(request):
    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                ...
                if not all([title, community_id, primary_methodology, planned_start_date, planned_end_date, objectives, description]):
    ...
    context = {
        'communities': OBCCommunity.objects.select_related('barangay__municipality__province').all(),
        'users': User.objects.filter(is_active=True).order_by('first_name', 'last_name'),
        'recent_assessments': Assessment.objects.select_related('community__barangay').order_by('-created_at')[:5],
    }
...
def create_workshop_activities(assessment, post_data):
    ...
    start_date = datetime.strptime(post_data.get('planned_start_date'), '%Y-%m-%d').date()
...
def assessment_detail(request, assessment_id):
    ...
    context = {
        'assessment': assessment,
        'workshops': assessment.workshop_activities.all().order_by('workshop_day', 'start_time'),
        'team_members': assessment.team_members.through.objects.filter(assessment=assessment).select_related('user'),
        'can_edit': request.user == assessment.created_by or request.user == assessment.lead_assessor,
    }
...
def workshop_detail(request, workshop_id):
    ...
    context = {
        ...
        'participants': workshop.participants.all().order_by('participant_type', 'name'),
        'outputs': workshop.outputs.all().order_by('output_type'),
        'can_edit': request.user == workshop.assessment.created_by or request.user in workshop.facilitators.all(),
    }
...
def generate_mana_report(request, assessment_id):
    ...
    if request.method == 'POST':
        ...
        report.social_development_findings = request.POST.get('social_development_findings', '')
        report.economic_development_findings = request.POST.get('economic_development_findings', '')
        report.cultural_development_findings = request.POST.get('cultural_development_findings', '')
        report.rights_protection_findings = request.POST.get('rights_protection_findings', '')
        ...
        report.program_development_opportunities = request.POST.get('program_development_opportunities', '')
