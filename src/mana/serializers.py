...
class AssessmentListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Assessment
        fields = [
            'id', 'title', 'category_name', 'community_name', 'status', 'priority',
            'planned_start_date', 'planned_end_date', 'actual_start_date', 'actual_end_date',
            'lead_assessor_name', 'duration_days', 'progress_percentage', 'created_at'
        ]
...
class NeedSerializer(serializers.ModelSerializer):
    ...
    category_name = serializers.CharField(source='category.name', read_only=True)
    assessment_title = serializers.CharField(source='assessment.title', read_only=True)
    community_name = serializers.CharField(source='assessment.community.name', read_only=True)
...
class NeedListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Need
        fields = [
            'id', 'title', 'category_name', 'priority_level', 'urgency_level',
            'assessment_title', 'community_name', 'estimated_beneficiaries',
            'resource_requirements', 'implementation_timeframe', 'status'
        ]
...
class BaselineStudySerializer(serializers.ModelSerializer):
    ...
    community_name = serializers.CharField(source='community.name', read_only=True)
    lead_researcher_name = serializers.CharField(source='lead_researcher.get_full_name', read_only=True)
...
class BaselineStudyListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = BaselineStudy
        fields = [
            'id', 'title', 'community_name', 'study_type', 'status', 'priority_level',
            'planned_start_date', 'planned_end_date', 'lead_researcher_name',
            'indicator_count', 'created_at'
        ]
