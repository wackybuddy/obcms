...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self._create_workshop_activities(assessments)
        ...
        self._print_summary(
            admin_user, facilitator1, facilitator2, participants, assessments
        )
    ...
    def _create_facilitators(self):
        ...
        for i in [1, 2]:
            ...
            if created:
                ...
                self.stdout.write(
                    f"  Created facilitator: test_facilitator{i} / password123"
                )
            else:
    ...
    def _create_participants(self, provinces, admin_user):
        ...
        for i in range(1, 121):
            ...
            if created:
                ...
                if i % 30 == 1:
                    self.stdout.write(
                        f"  Creating participants {i:03d} to {min(i+29, 120):03d}..."
                    )
            elif i == 1:
                self.stdout.write(
                    f"  Participants already exist, skipping creation messages..."
                )
    ...
    def _assign_facilitators(self, facilitator1, facilitator2, assessments):
        ...
        self.stdout.write(
            f"  Assigned facilitator1 to all {len(assessment_list)} assessments"
        )
    ...
    def _create_workshop_activities(self, assessments):
        """Create 5 workshop activities for each of the 4 assessments."""
        workshops = [
            ("workshop_1", "Workshop 1: Community Profile & Priority Needs"),
            ("workshop_2", "Workshop 2: Resource Mapping & Stakeholder Analysis"),
            ("workshop_3", "Workshop 3: Problem Tree & Root Cause Analysis"),
            ("workshop_4", "Workshop 4: Solution Design & Action Planning"),
            ("workshop_5", "Workshop 5: M&E Framework & Sustainability Plan"),
        ]

        for region, assessment in assessments.items():
            for i, (workshop_type, title) in enumerate(workshops, 1):
                WorkshopActivity.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "start_time": "09:00:00",
                        "end_time": "16:00:00",
                        "location": f"{assessment.province.name} Community Center",
                    },
                )
    ...
    def _print_summary(
        self, admin, facilitator1, facilitator2, participants, assessments
    ):
        ...
        self.stdout.write(
            f"  Facilitator 1: test_facilitator1 / password123 (All 4 assessments)"
        )
        self.stdout.write(
            f"  Facilitator 2: test_facilitator2 / password123 (First 2 assessments)"
        )
        ...
        for region, assessment in assessments.items():
            participant_count = WorkshopParticipantAccount.objects.filter(
                assessment=assessment
            ).count()
        ...
        self.stdout.write(
            f"  - 8 stakeholder types (elder, women_leader, youth_leader, etc.)"
        )

        self.stdout.write(self.style.WARNING(f"\n🎯 Workshop Activities:"))
        self.stdout.write(
            f"  - 5 workshops per assessment = 20 total workshop activities"
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "   - 4 regional assessments covering Regions IX, X, XII, XIII"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                "   - 30 participants per region for realistic cohort testing"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                "   - Facilitators can advance cohorts through workshops"
            )
        )
        self.stdout.write(
            self.style.SUCCESS(
                "   - Test notification system with large participant groups\n"
            )
        )
